...
from config import PUNE_LOCATIONS, API_CONFIG, DATABASE_CONFIG, REALTIME_CONFIG
...
class RealtimeDataCollector:
    ...
    def initialize_location_metadata(self):
        ...
        for loc_id, loc_config in PUNE_LOCATIONS.items():
            ...
            ''', (loc_id, loc_config.name, loc_config.lat, loc_config.lon,
                  loc_config.district, loc_config.zone, datetime.now().isoformat()))
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(API_CONFIG['open_meteo']['weather_url'], 
                    ...
                    if response.status == 200:
                        ...
                        self.logger.error(f"Weather API error for {location_id}: {response.status}")
                        return None
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(API_CONFIG['open_meteo']['air_quality_url'], 
                    ...
                    if response.status == 200:
                        data = await response.json()
                        processed_data = self.process_air_quality_data(data, location_id)
                        ...
                        self.logger.error(f"Air quality API error for {location_id}: {response.status}")
                        return None
    ...
    async def collect_all_locations_data(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        air_quality_results = await asyncio.gather(*air_quality_tasks, return_exceptions=True)
        ...
        try:
            ...
            for data in weather_data:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    data['timestamp'], data['location_id'], data['temperature'],
                    data['humidity'], data['pressure'], data['wind_speed'],
                    data['wind_direction'], data['precipitation'], data['solar_radiation'],
                    data['uv_index'], data['visibility'], data['cloud_cover'],
                    data['feels_like'], data['dew_point'], data['data_source'],
                    data['quality_score']
                ))
            ...
            for data in air_quality_data:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    data['timestamp'], data['location_id'], data['pm25'],
                    data['pm10'], data['no2'], data['so2'], data['co'], data['o3'],
                    data['aqi'], data['aqi_category'], data['dominant_pollutant'],
                    data['health_recommendation'], data['data_source'], data['quality_score']
                ))
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (datetime.now().isoformat(), 'all_locations', 'weather_air_quality', 
                  'success', len(weather_data) + len(air_quality_data)))
            
            conn.commit()
            self.logger.info(f"Saved {len(weather_data)} weather and {len(air_quality_data)} air quality records")
    ...
    async def run_continuous_collection(self):
        ...
        while True:
            try:
                ...
                self.logger.info(f"Collection cycle completed in {collection_time:.2f} seconds")
...
async def main():
    ...
    if weather_data or air_quality_data:
        ...
        print(f"✅ Collected {len(weather_data)} weather and {len(air_quality_data)} air quality records")
    else:
