...
from config import API_CONFIG, DATABASE_CONFIG, PUNE_LOCATIONS
...
class EnhancedDataCollector:
    ...
    async def fetch_weather_data(
        self, location_id: str, days_back: int = 7
    ) -> pd.DataFrame:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    API_CONFIG["open_meteo"]["weather_url"], params=current_params
                ) as response:
                    current_data = await response.json()

                # Fetch historical data
                async with session.get(
                    API_CONFIG["open_meteo"]["historical_url"], params=historical_params
                ) as response:
                    historical_data = await response.json()

            # Process and combine data
            weather_df = self.process_weather_data(
                current_data, historical_data, location_id
            )
            return weather_df
    ...
    async def fetch_air_quality_data(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # Fetch current data
                async with session.get(
                    API_CONFIG["open_meteo"]["air_quality_url"], params=current_params
                ) as response:
                    current_data = await response.json()

                # Fetch historical data
                async with session.get(
                    API_CONFIG["open_meteo"]["air_quality_url"],
                    params=historical_params,
                ) as response:
                    historical_data = await response.json()

            # Process and combine data
            aqi_df = self.process_air_quality_data(
                current_data, historical_data, location_id
            )
            return aqi_df
    ...
    def process_weather_data(
        self, current_data: Dict, historical_data: Dict, location_id: str
    ) -> pd.DataFrame:
        ...
        if "hourly" in historical_data:
            ...
            for i, timestamp in enumerate(times):
                ...
                record["quality_score"] = self.calculate_data_quality_score(record)
    ...
    def process_air_quality_data(
        ...
        if "hourly" in historical_data:
            ...
            for i, timestamp in enumerate(times):
                ...
                record["dominant_pollutant"] = self.calculate_dominant_pollutant(record)
                record["quality_score"] = self.calculate_data_quality_score(record)
    ...
    async def collect_all_locations_data(
        ...
        weather_df = (
            pd.concat(all_weather_data, ignore_index=True)
            if all_weather_data
            else pd.DataFrame()
        )
        aqi_df = (
            pd.concat(all_aqi_data, ignore_index=True)
            if all_aqi_data
            else pd.DataFrame()
        )
    ...
    def save_to_database(self, weather_df: pd.DataFrame, aqi_df: pd.DataFrame):
        ...
        try:
            ...
            if not weather_df.empty:
                weather_df.to_sql(
                    "weather_historical", conn, if_exists="append", index=False
                )
            ...
            if not aqi_df.empty:
                aqi_df.to_sql(
                    "air_quality_historical", conn, if_exists="append", index=False
                )
    ...
    async def run_data_collection(self, days_back: int = 7):
        ...
        if not weather_df.empty or not aqi_df.empty:
            ...
            self.logger.info(
                f"Data collection completed in {collection_time:.2f} seconds"
            )
            self.logger.info(
                f"Weather records: {len(weather_df)}, AQI records: {len(aqi_df)}"
            )
        else:
