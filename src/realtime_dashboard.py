...
class RealtimeDashboard:
    ...
    def create_location_map(self, data: Dict[str, pd.DataFrame]) -> go.Figure:
        ...
        for location_id, location_config in PUNE_LOCATIONS.items():
            ...
            if not data["weather"].empty:
                location_weather = data["weather"][
                    data["weather"]["location_id"] == location_id
                ]
        ...
        fig.update_layout(
            mapbox=dict(
                style="open-street-map", center=dict(lat=18.5204, lon=73.8567), zoom=10
            ),
            height=400,
    ...
    def create_time_series_chart(
        self, data: pd.DataFrame, y_column: str, title: str, color: str = "blue"
    ) -> go.Figure:
        ...
        if not data.empty and y_column in data.columns:
            for location_id in data["location_id"].unique():
                location_data = data[data["location_id"] == location_id].sort_values(
                    "timestamp"
                )
                location_name = (
                    PUNE_LOCATIONS.get(location_id, {}).name
                    if location_id in PUNE_LOCATIONS
                    else location_id
                )
    ...
    def create_prediction_chart(
        self, location_id: str, target_variable: str, days: int = 7
    ) -> go.Figure:
        ...
        try:
            ...
            if not historical_df.empty:
                ...
                if (
                    hasattr(self.ml_models, "models")
                    and target_variable in self.ml_models.models
                ):
                    future_dates = []
                    predictions = []
                    ...
                    for i in range(1, days + 1):
                        ...
                        try:
                            pred_result = self.ml_models.predict(
                                location_id, target_variable, horizon_days=i
                            )
        ...
        try:
            ...
            if not sensors_df.empty:
                ...
                status_df = sensors_df.merge(
                    recent_data_df, on=["location_id", "sensor_type"], how="left"
                )
    ...
    def run_dashboard(self):
        ...
        st.markdown(
            "Real-time monitoring and prediction of climate and air quality across Pune"
        )
        ...
        if selected_locations:
            for key in data:
                if not data[key].empty and "location_id" in data[key].columns:
                    data[key] = data[key][
                        data[key]["location_id"].isin(selected_locations)
                    ]
        ...
        with col1:
            ...
            if not data["weather"].empty:
                temp_fig = self.create_time_series_chart(
                    data["weather"], "temperature", "Temperature Over Time"
                )
        ...
        with col2:
            ...
            if not data["aqi"].empty:
                pm25_fig = self.create_time_series_chart(
                    data["aqi"], "pm25", "PM2.5 Over Time", "red"
                )
        ...
        with pred_col1:
            pred_location = st.selectbox(
                "Select Location for Predictions", selected_locations
            )
            pred_variable = st.selectbox(
                "Select Variable", ["temperature", "humidity", "pm25", "pm10", "aqi"]
            )
        ...
        if pred_location and pred_variable:
            pred_fig = self.create_prediction_chart(
                pred_location, pred_variable, pred_days
            )
