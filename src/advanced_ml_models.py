...
class AdvancedMLModels:
    ...
    def merge_datasets(
        self, weather_df: pd.DataFrame, aqi_df: pd.DataFrame, iot_df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        if not weather_df.empty and not aqi_df.empty:
            combined_df = pd.merge(
                weather_df, aqi_df, on=["timestamp", "location_id"], how="outer"
            )
    ...
    def feature_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "temperature" in df.columns and "humidity" in df.columns:
            ...
            df["dew_point"] = self.calculate_dew_point(
                df["temperature"], df["humidity"]
            )
        ...
        for col in numeric_columns:
            if col not in [
                ...
                df[f"{col}_lag24"] = df.groupby("location_id")[col].shift(
                    24
                )  # 24 hours ago
    ...
    def prepare_training_data(
        ...
        feature_columns = [
            ...
            if col not in exclude_columns + [target_variable]
        ]

        # Handle missing values
        df_clean[feature_columns] = df_clean[feature_columns].fillna(
            df_clean[feature_columns].median()
        )
    ...
    def train_ensemble_model(
        self, X: np.ndarray, y: np.ndarray, target_variable: str
    ) -> Dict[str, Any]:
        ...
        weights = {}
        total_r2 = sum(
            scores["r2"] for scores in model_scores.values() if scores["r2"] > 0
        )
        ...
        self.logger.info(
            f"Ensemble - MAE: {ensemble_mae:.4f}, MSE: {ensemble_mse:.4f}, R2: {ensemble_r2:.4f}"
        )
    ...
    def train_lstm_model(
        ...
        if len(X_seq) == 0:
            self.logger.warning(
                f"Not enough data for LSTM training for {target_variable}"
            )
            return {}
    ...
    def train_all_models(self, days_back: int = 365):
        ...
        for target_variable in MODEL_CONFIG["target_variables"]:
            ...
            X, y, feature_columns = self.prepare_training_data(
                df_features, target_variable
            )

            if len(X) < 100:  # Minimum data requirement
                self.logger.warning(
                    f"Insufficient data for {target_variable}: {len(X)} samples"
                )
                continue
            ...
            if "random_forest" in ensemble_results.get("models", {}):
                ...
                self.feature_importance[target_variable] = dict(
    ...
    def predict(
        self, location_id: str, target_variable: str, horizon_days: int = 1
    ) -> Dict[str, Any]:
        ...
        if lstm_results and "model" in lstm_results:
            ...
            if len(recent_features) >= sequence_length:
                # Prepare sequence
                sequence_data = (
                    recent_features[feature_columns].tail(sequence_length).values
                )
                scaler_X = lstm_results["scaler_X"]
                scaler_y = lstm_results["scaler_y"]

                X_seq = scaler_X.transform(sequence_data).reshape(
                    1, sequence_length, -1
                )
    ...
    def get_recent_data_for_prediction(
        self, location_id: str, days_back: int = 30
    ) -> pd.DataFrame:
    ...
    def save_models(self):
        ...
        for target_variable, model_info in self.models.items():
            ...
            if "lstm" in model_info and "model" in model_info["lstm"]:
                ...
                lstm_meta = {
                    k: v for k, v in model_info["lstm"].items() if k != "model"
                }
