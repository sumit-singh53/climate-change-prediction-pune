...
class SystemOrchestrator:
    ...
    def run_dashboard(self):
        ...
        try:
            ...
            dashboard_path = os.path.join(
                os.path.dirname(__file__), "realtime_dashboard.py"
            )
    ...
    async def start_initial_data_collection(self):
        ...
        try:
            ...
            weather_data, air_quality_data = (
                await self.realtime_collector.collect_all_locations_data()
            )

            if weather_data or air_quality_data:
                self.realtime_collector.save_to_database(weather_data, air_quality_data)
                self.logger.info(
                    f"Initial data collection completed: {len(weather_data)} weather, {len(air_quality_data)} air quality records"
                )
            else:
    ...
    def start_all_services(self, collect_initial_data: bool = True):
        ...
        realtime_thread = threading.Thread(
            target=lambda: asyncio.run(self.run_realtime_collection()), daemon=True
        )
        realtime_thread.start()

        # Start historical data collection service
        data_collection_thread = threading.Thread(
            target=lambda: asyncio.run(self.run_data_collection()), daemon=True
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Climate & AQI Prediction System"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--no-dashboard", action="store_true", help="Skip dashboard startup"
    )
    ...
    try:
        if args.mode == "full":
            orchestrator.start_all_services(
                collect_initial_data=args.collect_initial_data
            )
